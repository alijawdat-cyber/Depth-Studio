...
## 🏢 **مجموعة البراندات (brands)**

### 📋 **هيكل البراند الأساسي**
**Collection Path:** `brands/{brandId}`

```javascript
{
  id: string, // معرف فريد للبراند
  name: {
    ar: "اسم البراند بالعربية", // إجباري
    en: "Brand Name in English" // اختياري
  },
  // ... existing code ...
  
  // ✅ تأكد: البراندات تُضاف ديناميكياً حسب الحاجة
  // ✅ لا توجد براندات أولية مُسبقة في النظام
  created_at: timestamp,
  created_by: "علي جودت", // المدير العام
  updated_at: timestamp,
  updated_by: string
}
```

### 🚀 **نظام البراندات المرن**
- **بدء النظام:** Collection فارغ تماماً
- **إضافة البراندات:** فقط عند الحاجة الفعلية للعمل
- **مرونة كاملة:** إضافة/تعديل/حذف أي براند في أي وقت
- **تخصيص منسق:** عند إنشاء كل براند جديد